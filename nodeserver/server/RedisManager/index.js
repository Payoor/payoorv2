...
class RedisManager {
  ...
  async setJSON (key, item, expirationSeconds = null, keepTTL = false) {
    ...
    try {
      ...
      if (keepTTL) {
        ...
        console.log(
          `✅ Set item: '${key}' with expiration ${expirationSeconds}s`
        )
      } else {
  ...
  async sismember (key, member) {
    ...
    try {
      ...
      console.log(
        `🔎 Checked if '${member}' is a member of set '${key}': ${
          isMember ? 'Yes' : 'No'
        }.`
      )
      return isMember === 1
      ...
      console.error(
        `❌ Error checking sismember for key '${key}' and member '${member}':`,
        error
      )
      throw error
    }
  ...
  async hset (key, field, value) {
    ...
    try {
      ...
      console.log(
        `✅ Set hash field '${field}' in key '${key}'. Result: ${result}`
      )
      return result
    } catch (error) {
      console.error(
  ...
  async hgetall (key) {
    ...
    try {
      ...
      console.log(
        `🔎 Retrieved all fields from hash '${key}'. Found ${
  ...
  async deleteItem (...keys) {
    ...
    try {
      ...
      console.log(
        `🗑️ Deleted ${deletedCount} item(s) from Redis: ${keys.join(', ')}`
      )
      return deletedCount
    } catch (error) {
      console.error(
        `❌ Error deleting item(s) for keys '${keys.join(', ')}':`,
        error
      )
      throw error
    }
