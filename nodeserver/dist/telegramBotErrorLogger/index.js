...
async function telegramBotErrorLogger(errorMessage) {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      console.error(`Failed to send error message to telegbot. Status: ${response.status}, Body: ${response.data}`);
      fs.appendFile(errorLogPath, errorMessage, fileErr => {
        if (fileErr) {
          console.error('Failed to write error to fallback log file:', fileErr);
        } else {
          console.log(`Error successfully logged to fallback file: ${errorLogPath}`);
        }
      });
    }
  } catch (error) {
...
var _default = exports.default = telegramBotErrorLogger;