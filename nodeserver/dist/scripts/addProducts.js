...
const createProductsIndexIfMissing = async () => {
  try {
    ...
    const existsResponse = await _axios.default.head(`${baseUrl}/${INDEX_NAME}`).catch(err => {
    ...
    const response = await _axios.default.put(`${baseUrl}/${INDEX_NAME}`, {
      mappings: {
        properties: {
          _mongooseid: {
            type: 'keyword'
...
const bulkIndexProducts = async products => {
  ...
  try {
    const response = await _axios.default.post(`${baseUrl}/_bulk`, bulkBody, {
      headers: {
    ...
    await _axios.default.post(`${baseUrl}/${INDEX_NAME}/_refresh`);
    console.log(`✅ Refreshed "${INDEX_NAME}" index`);
  } catch (error) {
    console.error('❌ Error bulk indexing products:', error.response ? error.response.data : error.message);
    throw error;
  }
};
const run = async () => {
  try {
    await _mongoose.default.connect(MONGO_URL);
