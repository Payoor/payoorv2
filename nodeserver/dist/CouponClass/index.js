...
class CouponClass {
  static async createCouponType(type, ttlInSeconds, discount = {}) {
    ...
    if (typeof discount !== 'object' || !discount.percentage && !discount.flat && !discount.freeDelivery) {
    ...
    const exists = await _redisconf.redisClient.exists(key);
    if (exists) {
    ...
    await _redisconf.redisClient.set(key, JSON.stringify(config));

    // 👉 Add to set of coupon types
    await _redisconf.redisClient.sadd('coupon:types', type);
    return {
      success: true,
  ...
  static async createCoupon(type) {
    ...
    const typeData = await _redisconf.redisClient.get(typeKey);
    if (!typeData) {
    ...
    await _redisconf.redisClient.set(couponKey, JSON.stringify(metadata), 'EX', ttl);
    await _redisconf.redisClient.sadd(`coupon:type:${type}:codes`, code);
    return {
      code,
      type,
      expiresIn: ttl
  ...
  static async getCoupon(code) {
    const couponKey = `coupon:code:${code}`;
    const raw = await _redisconf.redisClient.get(couponKey);
    if (!raw) return null;
  ...
  static async redeemCoupon(code) {
    ...
    await _redisconf.redisClient.set(couponKey, JSON.stringify(coupon), {
      KEEPTTL: true
    });
    return {
      success: true,
  ...
  static async listCoupons(type) {
    const codes = await _redisconf.redisClient.smembers(`coupon:type:${type}:codes`);
    return codes;
  }
  static async getCouponTypeConfig(type) {
    const typeKey = `coupon:type:${type}`;
    const raw = await _redisconf.redisClient.get(typeKey);
