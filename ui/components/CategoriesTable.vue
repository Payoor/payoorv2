                                    ...
                                    <div>
                                        ...
                                        <template v-if="header.key === 'actions'">
                                            <div class="admintable__actions">
                                                ...
                                                <button
                                                    v-if="focusedItem._id === item._id && item._id === `0` && item.image && item.name && item.description && item.hexcolor"
...
export default {
    ...
    methods: {
        ...
        async saveItemToDB(itemToSave) {
            try {
                const res = await fetch(`${serverurl}/admin/update-category/${itemToSave._id}`, {
                    method: 'PUT',
        ...
        async saveNewItemToDB(item) {
            try {
                ...
                localItems[0] = category
                this.localItems = localItems

                this.finalizeSaveSnapshot(category)
        ...
        async deleteProduct(itemId) {
            ...
            try {
                const res = await fetch(`${serverurl}/admin/delete-category/${itemId}`, {
                    method: 'DELETE',
                    headers: this.getAuthHeaders()
                });
